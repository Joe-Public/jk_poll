...
*/			$template['answers'] = $this->cObj->substituteMarkerArrayCached($template['answers'],$markerArray,$subpartArray,$wrappedSubpartArray);
			...
			foreach ($answers as $i => $a) {
				...
				// get typoscript color definitions
				if ($this->conf['show_css_bars'] || $this->pi_getFFvalue($this->cObj->data['pi_flexform'],'show_css_bars','s_result')) {
					if ($this->conf['cssColors.'][trim($colors[$i])]) {
						$csscolor = $this->conf['cssColors.'][trim($colors[$i])];
					} else {
						$csscolor = "blue";
					}
				}
				if ($type == 0) {
					//horizontal
					if ($this->conf['show_css_bars'] || $this->pi_getFFvalue($this->cObj->data['pi_flexform'],'show_css_bars','s_result')) {
						$markerArrayAnswer["###IMG_PERCENTAGE_RESULT###"] = '<div style="float:left; background-color:'.$csscolor.'; width:'.$bar.'px; height:'.$height_width.'px;" title="'.$percent.'%"></div>';
					} else {
					...
					// vertical
					if ($this->conf['show_css_bars'] || $this->pi_getFFvalue($this->cObj->data['pi_flexform'],'show_css_bars','s_result')) {
						$markerArrayAnswer["###IMG_PERCENTAGE_RESULT###"] = '<div style="height:'.($max*$factor).'px; width:'.$height_width.'px;"><div style="position:relative; top: '.($max*$factor-$percent*$factor).'px; bottom:0px; background-color:'.$csscolor.'; width:'.$height_width.'px; height:'.$bar.'px;"></div></div>';
					} else {
